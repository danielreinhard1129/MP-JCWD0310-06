      ...
      <thead {...headerProps}>
        <tr className="flex">
          {weekDays.map((day, index) => (
            <th
              className="w-9 rounded-xl text-[0.8rem] font-normal text-muted-foreground"
              key={index}
            >
              {day}
            </th>
...
function TimeField(props: AriaTimeFieldProps<TimeValue>) {
  ...
  return (
    <div
      ...
      ref={ref}
      className={cn(
        'inline-flex h-full w-fit flex-1 rounded-md border border-input bg-transparent px-3 py-2',
        props.isDisabled && 'cursor-not-allowed opacity-50',
      )}
    >
      {state.segments.map((segment, i) => (
